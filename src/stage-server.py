...
#
# This a server for the 4 Standa Stages used in the STUF project
...
async def main(HOST, PORT):
    log.info("Opening connection @"+HOST+":"+str(PORT))
...
if __name__ == "__main__":
    ...
    log.info("Number of devices = "+str(dev_count))
    try:
        log.info("List of devices = ")
        all_device_check = True
        for i in dev_list:
            print(repr(i))
            # Collector Rotary Stage
            if '49E5' in repr(i):
                axis_a = Stage(lib, i, "Collector Rotary Stage")
                if axis_a.stageDev > 0:
                    open_devs[0] = axis_a
                else:
                    all_device_check = False
                    log.error("BAD = Collector Rotary Stage connection failed")
